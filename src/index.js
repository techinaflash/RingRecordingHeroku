...
app.get('/screenshotweb', (req, res) => { 
  screenshotlayer.captureWeb().then((result) => { 
    ...
    var screenshot = result.data
      ...
      const uploadresult = web.files.post({
      ...
      //curl -F file=screenshot -F filetype='png' -F filename='TicketDashboard.png' -F initial_comment='Ticket Dashboard Screnshot' -F channels=conversationId -H Authorization=process.env.SLACK_ACCESS_TOKEN https://slack.com/api/files.upload ;
  }) 
})
